/**
 * CSS color parsing -> docx hex color strings (6 hex digits, no '#').
 */

// Standard CSS3/SVG named colors.
const NAMED_COLORS = {
...
/**
 * Converts a CSS color value (hex, rgb()/rgba(), or named) to a 6-digit
 * uppercase hex string with no leading '#', suitable for docx `color`/
 * `fill` properties. Returns null if the value can't be parsed (e.g.
...
export function cssColorToHex(value) {
  ...
  if (lower === 'transparent' || lower === 'inherit' || lower === 'initial' ||
      lower === 'currentcolor' || lower === 'unset') {
    return null;
  }
